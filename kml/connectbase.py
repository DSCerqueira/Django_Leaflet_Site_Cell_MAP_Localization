...
def exportgjson(element):
    ...
    #query="select ExportGeoJSON2('sectors', 'geometry', 'mytable.geojson')"
    #query = "SELECT 'sites' FROM sectors WHERE ST_MaxX(geometry) != maxx"
...
def rungeojson():
    ...
    con.enable_load_extension(True)
    con.load_extension('mod_spatialite')
    query='select' + "'" + '{ "type" : "Feature", "properties" : { "sector":' + "'" + '|| sector ||' +"'" +',"phycellid":' + "'" +' || phycellid||' + "'" + '},'+"'"+' || AsGeoJSON(geometry) ||'+"'"+'},'+"'"+' as concated from sectortemp;'
    print(query)
    cur.execute(query)
    table = pd.DataFrame(cur.fetchall())
    print(table)
